...
#define UART3_BASE		(RPi_IO_Base_Addr + 0x00201600)
...
#define UART3_DR		(*(volatile uint32_t*)(UART3_BASE + 0x00))
...
// bit 1:	 PE			パリティエラー
// bit 0:	 FE			フレーミングエラー
#define UART3_RSRECR	(*(volatile uint32_t*)(UART3_BASE + 0x04))
...
// bit 0:	 CTS		送信クリア
#define UART3_FR		(*(volatile uint32_t*)(UART3_BASE + 0x18))

/* 整数ボーレート除数レジスタ */
// bit31-16:			予約 0固定
// bit15- 0: IBRD		バンドレートディバイダー(整数部分)
//						IBRD = 3,000,000/(16*Baudrate)
#define UART3_IBRD		(*(volatile uint32_t*)(UART3_BASE + 0x24))

/* 小数ボーレート除数レジスタ */
// bit31- 6:			予約 0固定
// bit 5- 0: FBRD		バンドレートディバイダー(小数部分)
//						FBRD = (24,000,000/Baudrate-IBRD*128+1)/2
#define UART3_FBRD		(*(volatile uint32_t*)(UART3_BASE + 0x28))
...
#define UART3_LCRH		(*(volatile uint32_t*)(UART3_BASE + 0x2C))
...
#define UART3_CR		(*(volatile uint32_t*)(UART3_BASE + 0x30))
...
#define UART3_IFLS		(*(volatile uint32_t*)(UART3_BASE + 0x34))
...
// bit 0:	 RIMIM		未サポート
#define UART3_IMSC		(*(volatile uint32_t*)(UART3_BASE + 0x28))
...
// bit 0:	 RIMRIS		未サポート
#define UART3_RIS		(*(volatile uint32_t*)(UART3_BASE + 0x3C))
...
// bit 0:	 RIMMIS		未サポート
#define UART3_MIS		(*(volatile uint32_t*)(UART3_BASE + 0x40))
...
// bit 0:	 RIMIC		未サポート
#define UART3_ICR		(*(volatile uint32_t*)(UART3_BASE + 0x44))

#define UART3_ITCR		(*(volatile uint32_t*)(UART3_BASE + 0x80))
#define UART3_ITIP		(*(volatile uint32_t*)(UART3_BASE + 0x84))
#define UART3_ITOP		(*(volatile uint32_t*)(UART3_BASE + 0x88))
#define UART3_TDR		(*(volatile uint32_t*)(UART3_BASE + 0x8C))

uint32_t uart3_init(uint32_t Baud);

void uart3_putc(char byte);
