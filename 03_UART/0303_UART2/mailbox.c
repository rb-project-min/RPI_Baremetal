...
uint32_t mailbox_GetCurrentClock(uint32_t ClockID)
{
	mbox[0] = 8 * 4;								// バッファサイズ(ヘッダーからパディングまで)
	mbox[1] = RPI_FIRMWARE_STATUS_REQUEST;			// リクエスト/レスポンス

	mbox[2] = RPI_FIRMWARE_GET_CLOCK_RATE;			// タグ
	mbox[3] = 8;									// バッファサイズ(このタグの応答のみ)
	mbox[4] = RPI_FIRMWARE_STATUS_REQUEST;			// リクエスト/レスポンス
	mbox[5] = ClockID;
	mbox[6] = 0;
...
uint32_t GetUartCurrentClock()
{
	return mailbox_GetCurrentClock(0x00000002);
}

uint32_t GetCoreCurrentClock()
{
	return mailbox_GetCurrentClock(0x00000004);
}
